...
import { CUSTOM, CUSTOM_ELEMENT, CUSTOM_ELEMENT_TYPES, ELEMENT_TYPES, EXPAND_ELEMENTS} from './blockTemplates.constants';
import {
...
const getCustomElementDropdown = () => index => $(`div#elements\\.0\\.params\\.elements\\.${index}`);

const getCustomElementNameInput = () => index => $(`#elements\\.0\\.params\\.elements\\.${index}\\.name`);

const getElementTypeSelect = () => (elementType, index, elementName) =>
  $(
    `#${camelize(elementType)}-element-container-${index} #${camelize(elementType)}-element-${camelize(elementName)}-${
      ELEMENT_TYPES[camelize(elementName)]
    }`
  );
...
const selectCustomElement = CreateBlockTemplate => (elementTypeName, index) => {
  ...
  clickElement(getCustomElementDropdown()(index));
  getElementTypeSelect()(CUSTOM, index, elementTypeName).scrollIntoView();
  clickElement(getElementTypeSelect()(CUSTOM, index, elementTypeName));
  // const elementTypeSelectName = getCustomElementDropdown()(index).getText();
  setValue(getCustomElementNameInput()(index), getCustomElementDropdown()(index).getText());
