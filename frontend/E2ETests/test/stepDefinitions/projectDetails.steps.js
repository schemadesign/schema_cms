...
Given('I have entered project details page', () => {
  ...
  expect(ProjectDetailsPage.TopHeader.projectName.getText()).to.equal(
...
Then(/^I can see project details page with all its content$/, function() {
  ...
  assert(
    ProjectDetailsPage.usersTile.isDisplayed(),
    'Users tile is not displayed'
  );
  assert(
    ProjectDetailsPage.usersTileAmount.isDisplayed(),
    'Users amount is not displayed'
  );
  expect(ProjectDetailsPage.usersTileLabel.getText()).to.equal(USERS_LABEL);

  assert(
    ProjectDetailsPage.dataSourcesTile.isDisplayed(),
    'Data Sources tile is not displayed'
  );
  assert(
    ProjectDetailsPage.dataSourcesTileAmount.isDisplayed(),
    'Data Sources amount is not displayed'
  );
  expect(ProjectDetailsPage.dataSourcesTileLabel.getText()).to.equal(
    DATA_SOURCES_LABEL
  );

  assert(
    ProjectDetailsPage.chartsTile.isDisplayed(),
    'Charts tile is not displayed'
  );
  assert(
    ProjectDetailsPage.chartsTileAmount.isDisplayed(),
    'Charts amount is not displayed'
  );
  expect(ProjectDetailsPage.chartsTileLabel.getText()).to.equal(CHARTS_LABEL);

  assert(
    ProjectDetailsPage.pagesTile.isDisplayed(),
    'Pages tile is not displayed'
  );
  assert(
    ProjectDetailsPage.pagesTileAmount.isDisplayed(),
    'Pages amount is not displayed'
  );
  ...
  expect(ProjectDetailsPage.lastUpdateLabel.getText()).to.equal(
    LAST_UPDATE_LABEL
  );
  ...
  expect(ProjectDetailsPage.descriptionLabel.getText()).to.equal(
    DESCRIPTION_LABEL
  );
  ...
  assert(
    ProjectDetailsPage.backBtn.isDisplayed(),
...
Then(/^tabs are displayed$/, function() {
  ...
  assert(
    ProjectDetailsPage.Tabs.settings.isDisplayed(),
    'Settings tab is not displayed'
  );
  assert(
    ProjectDetailsPage.Tabs.sources.isDisplayed(),
    'Sources tab is not displayed'
  );
  assert(
    ProjectDetailsPage.Tabs.users.isDisplayed(),
    'Settings tab is not displayed'
  );
  assert(
    ProjectDetailsPage.Tabs.pages.isDisplayed(),
    'Settings tab is not displayed'
  );
});
