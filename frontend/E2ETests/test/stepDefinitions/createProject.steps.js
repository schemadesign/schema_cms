...
Then('created project is at the top of the list', () => {
  ...
  expect(ProjectsPage.projectTitle.getText()).to.equal(
    CREATE_PROJECT_VALID_TITLE
  );
});
...
Then('validation message about empty fields is displayed', () => {
  ...
  expect(CreateProjectPage.titleErrorMsg.getText()).to.equal(
    CREATE_PROJECT_EMPTY_FIELD_ERROR
  );
  expect(CreateProjectPage.descriptionErrorMsg.getText()).to.equal(
    CREATE_PROJECT_EMPTY_FIELD_ERROR
  );
});
...
Then(
  /^all elements of Create New Project page are displayed properly$/,
  function() {
    ...
    expect(CreateProjectPage.finishBtn.getText()).to.equal(
      CREATE_PROJECT_SUBMIT_LABEL
    );
    expect(CreateProjectPage.cancelBtn.getText()).to.equal(
      CREATE_PROJECT_CANCEL_LABEL
    );
    expect(CreateProjectPage.statusDropdown.getText()).to.equal(
      CREATE_PROJECT_DEFAULT_STATUS
    );
    expect(CreateProjectPage.statusLabel.getText()).to.equal(
      CREATE_PROJECT_STATUS_LABEL
    );
    expect(CreateProjectPage.ownerLabel.getText()).to.equal(
      CREATE_PROJECT_OWNER_LABEL
    );
    expect(
      CreateProjectPage.descriptionInput.getAttribute('placeholder')
    ).to.equal(CREATE_PROJECT_DESCRIPTION_PLACEHOLDER);
    expect(CreateProjectPage.descriptionLabel.getText()).to.equal(
      CREATE_PROJECT_DESCRIPTION_LABEL
    );
    expect(CreateProjectPage.titleInput.getAttribute('placeholder')).to.equal(
      CREATE_PROJECT_TITLE_PLACEHOLDER
    );
    expect(CreateProjectPage.SubHeader.title.getText()).to.equal(
      CREATE_PROJECT_HEADER_TITLE
    );
    expect(CreateProjectPage.titleLabel.getText()).to.equal(
      CREATE_PROJECT_TITLE_LABEL
    );
    expect(CreateProjectPage.SubHeader.subtitle.getText()).to.equal(
      CREATE_PROJECT_HEADER_SUBTITLE
    );
  }
);
...
Then(/^Finish button is not active$/, function() {
  ...
  assert(
    CreateProjectPage.finishBtn.isEnabled() === false,
