...
class DataSourceJobFSM(models.Model):
    ...
    @django_fsm.transition(
        job_state, source=[constants.DataSourceJobState.PENDING], target=constants.DataSourceJobState.SUCCESS
    )
    def success(self):
        self.update_meta()
    ...
    @django_fsm.transition(
        job_state, source=[constants.DataSourceJobState.PENDING], target=constants.DataSourceJobState.FAILED
    )
    def fail(self):
        pass
