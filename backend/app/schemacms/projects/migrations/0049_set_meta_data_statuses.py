...
def old_meta_data_to_success(apps, schema_editor):
    ...
    with transaction.atomic():
        MetaData.objects.using(db_alias).filter(status="pending").update(status="success")
        ...
        for data_source in failed_data_sources:
            MetaData.objects.using(db_alias).create(datasource=data_source, status="failed")
