...
class TagsList(
    ...
    def meta_file_serialization(self):
        data = dict(
            id=self.id,
            name=self.name,
            tags=[tag.meta_file_serialization() for tag in self.tags.all()],
        )
        return data
...
class Tag(
    ...
    tags_list: TagsList = models.ForeignKey(
        TagsList, on_delete=models.CASCADE, related_name="tags"
    )
...
class State(
    ...
    project = models.ForeignKey(
        "projects.Project", on_delete=models.CASCADE, related_name="states"
    )
