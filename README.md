...
2. Go to `Domains` tab and click `Verify a New Domain` button.
3. Enter a domain name which you verified in [Deploy Certificates](#deploy-certificates) step.
4. AWS will generate `Domain Verification Record` which has to be added in your DNS Manager.
    - choose `TXT` as `type` of this record
    - enter `_amazonses` as record name
    - enter value generated by AWS as record value
5. When domain `Verification Status` will change to `verified` you have to [request](https://docs.aws.amazon.com/ses/latest/DeveloperGuide/request-production-access.html) moving out from `Sandbox` mode.
